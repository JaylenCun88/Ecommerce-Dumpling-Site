...
function toProduct(row: ProductRow): Product {
  return { slug: row.slug, name: row.name, region: row.region, country: row.country, price: row.price_cents / 100, description: row.description, ingredients: row.ingredients, packageSize: row.package_size, dietary: row.dietary, tone: row.tone, accent: row.accent, shape: row.shape };
}
